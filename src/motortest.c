	...
	while (1)
	{
		...
		switch (d)
		{
			...
			case MOTOR_RESET:
				...
				printf("x_max_steps=%d,y_max_steps=%d,x_cur_step=%d,y_cur_step=%d\n", motor_reset_data.x_max_steps,
						motor_reset_data.y_max_steps, motor_reset_data.x_cur_step, motor_reset_data.y_cur_step);
				break;
			case MOTOR_MOVE:
				printf("steps x：");
				scanf("%d", &a);
			...
			case MOTOR_SPEED:
				printf("speed：");
				scanf("%d", &e);
				motor_action.motor_move_speed = e;
				ioctl(fd, d, (unsigned long) &motor_action);
				break;
			case MOTOR_GOBACK:
				ioctl(fd, d, 0);
				break;
